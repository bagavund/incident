...
> Версия анализа: 2026-09-10 · составлено по фактическому состоянию кода.
...
- Опубликовать инцидент можно только с полным набором: название, сервис(ы), тип,
  критичность, дежурный, время начала, постмортем. Незаполненные обязательные
  поля форма подсвечивает и проматывает к первому из них.
- Один инцидент может затрагивать несколько сервисов одновременно.
- **Черновик:** сохраняется по одному названию — остальное дозаполняется позже.
  Помечается в списке, данные не нужно выдумывать ради сохранения.
...
flowchart LR
    ...
    DB[("MySQL 8<br/>15 миграций<br/>драйвер меняется в .env")]
...
`backend` / `frontend`; в одном репо они разведены по каталогам, у каждого свой
Dockerfile. Vite собирает статику в `frontend/dist`, nginx отдаёт её на все
пути, кроме `/api/`, который уходит на бэкенд по внутреннему адресу
`BACKEND_URL`. Локально ту же роль играет dev-прокси Vite.
...
**Оформление — дизайн-система Азбуки Вкуса.** Один шрифт Golos Text,
приглушённая серая палитра (Grey 10–90), зелёный акцент точечно; статус
несут отдельные цвета (красный / золото / синий), зелёный означает
«хорошо». Светлая тема по умолчанию, тёмная — переключателем в
«Настройках»; палитра переопределяется поверх одних и тех же
utility-классов через `:root:not([data-theme='dark'])` / `[data-theme='dark']`
в `src/index.css`, тема проставляется до первого кадра инлайн-скриптом в
`index.html`. KPI дашборда — одной делёной полосой, число столбцов по
факту метрик; скругление крайних ячеек полосы задаётся в `src/index.css`
(а не `overflow:hidden` на контейнере — иначе он обрезал бы всплывающие
подсказки `(i)`). В карточке инцидента «Начало инцидента» — первый пункт
хронологии; правый блок «Тайминги» свёлся к карточке «Дежурный» (строка
«Обнаружен» убрана — этот момент и так есть в хронологии шагом «Обнаружено»).
...
- `title` — строка; `type`, `criticality` — nullable-строки (у черновика могут быть пустыми, при публикации обязательны и сверяются со справочником)
...
кастами, и одни и те же данные кодируются по-разному: JSON-поля (`zones`,
`impact_targets`) MySQL хранит с пробелом после запятой, а энкодер Laravel —
без; `boolean` из БД приходит `0/1`, а каст даёт `false/true`. Поэтому
`IncidentAuditor::normalize()` сворачивает обе стороны к канону перед
сравнением (JSON остаётся декодированным — журнал хранит массивы, не строки),
иначе на каждое сохранение падала ложная запись «поле → то же». Разовая
миграция `2026_09_10_000001_prune_phantom_json_diffs_from_incident_audits`
вычистила такие пары из уже накопленных записей. Клиент
(`IncidentDetail.tsx · formatAuditValue`) при выводе очеловечивает значения:
даты → «9 сентября 2026, 07:00», enum'ы и площадки влияния → подписи.
...
- **Пароли** — bcrypt (`Hash`), `password_confirmation`. `Password::defaults()`:
  ...
  базе утечек (`uncompromised`); `max(72)` (предел bcrypt) в обеих ветках.
  Форма показывает подсказку о требованиях под полем. Смена своего пароля
  требует текущий и отзывает старый токен; админ может сбросить пароль
  пользователя.
...
| Показатель | Значение |
|---|---|
| Feature-тестов (backend) | ~56 в `tests/Feature` (Auth, Incident, Lookup, Sla, User, Analytics) |
| Миграций БД | 15 |
...
`stub_duration` / escalation-spans, журнал аудита (дифф полей, no-op, отсутствие
ложных диффов по JSON/boolean-полям, чистящая миграция, запись переживает
удаление), CRUD справочников (регистронезависимые дубли, `409` на
...
(осознанный пробел). Разовый прогон `knip` мёртвого кода и лишних зависимостей
не выявил. БД тестов — отдельная схема `ims_testing` (MySQL).

### 10. Эксплуатация

Один репозиторий, две самодостаточные части — `backend/` (Laravel, только API) и
`frontend/` (React SPA + nginx), у каждой свой Dockerfile.
...
*IMS · Системный анализ · по фактическому состоянию кода на 2026-09-10*
