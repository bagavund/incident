...
> Версия анализа: 2026-09-08 · составлено по фактическому состоянию кода.
...
редактирует только свои). Развёртывание — один контейнер на одной ВМ.
...
Классический SPA поверх REST API, рассчитанный на деплой одним процессом на
...
одной ВМ. Vite собирает статику в `backend/public`, Laravel отдаёт её catch-all
маршрутом для всех путей, кроме `/api/*`. Это убирает CORS в проде: всё на одном
origin.
...
| FormRequest | валидация входа + авторизация | `IncidentRequest`, `LoginRequest`, `CreateUserRequest` |
...
**Фронтенд (`src/`, React 18 + TS).**

| Модуль | Ответственность |
|---|---|
| `lib/api.ts` | типизированный HTTP-клиент: подставляет токен, парсит ошибки в `ApiError`, ловит 401 → выход |
| `adapters.ts` | преобразование `snake_case` API ⇄ модель экранов; enum-`value` ⇄ подпись |
| `store.tsx` | `StoreProvider` — при монтировании тянет справочники, пользователей, порог SLA, весь список инцидентов; отдаёт CRUD-интерфейс |
| `auth.tsx` | сессия: логин, восстановление по токену через `/auth/me`, `isAdmin`, `canEditIncident()` |
...
**`cache`** — стандартная таблица Laravel; используется как денилист отозванных
JWT (`jti` → до истечения `exp`).
...
`LOWER()` в SQLite сворачивает только ASCII — «Значение А» и «значение а» в SQL не
совпадали бы. Сравнение вынесено в PHP (`mb_strtolower`), воспроизведено тестом.
...
`leeway` 30 с. `logout` / `refresh` кладут `jti` старого токена в кэш до его
`exp`; `JwtAuthenticate` проверяет денилист на каждом запросе. `login` отвечает
`422` на неверную пару (не `401`) — поэтому `401` на клиенте всегда однозначно
«токен непригоден» → выход.

### 6. API

31 маршрут под `/api`. Публичный только `POST /auth/login`; остальные требуют
`Authorization: Bearer <token>` (middleware `auth.jwt`). Запись в справочники,
SLA и пользователей — дополнительно `role:admin`.

**Авторизация**

| Метод | Путь | Доступ | Назначение |
|---|---|---|---|
| POST | `/auth/login` | публичный | Выдача JWT по логину/паролю |
| GET | `/auth/me` | авторизованный | Текущий пользователь |
| POST | `/auth/refresh` | авторизованный | Обмен токена на новый (старый в денилист) |
...
(включая `stub_duration` и `escalation.*`); `src/data.ts` содержит только
...
- **Аутентификация** — собственный JWT (firebase/php-jwt) + денилист в кэше.
  Временное решение до корпоративного SSO; экран входа изолирован.
...
- **Пароли** — bcrypt (`Hash`), подтверждение при заводе (`password_confirmation`).
- **Известное ограничение** — фиксированные `APP_KEY`/`JWT_SECRET` в
  `docker-compose.yml` только для локального запуска; для любого не-локального
  окружения генерируются свои.

### 9. Тесты и качество

| Показатель | Значение |
|---|---|
| Тестов PHPUnit (backend) | 59 — все проходят (248 ассертов) |
| Миграций БД | 16 |
| Маршрутов REST API | 31 |
| Замечаний Pint / `tsc -b` | 0 |
...
удаление используемого), полный payload дашборда. Frontend — ручная проверка
ключевых сценариев; юнит-тестов на фронте нет (осознанный пробел).
...
Тёмная тема, приглушённые границы, зелёный акцент точечно, моноширинный шрифт для
чисел и времени. Скриншоты сняты со свежего сида (`scripts/screenshots.mjs`,
Playwright), исходники — `docs/screenshots/`.

**Вход** — единственный публичный экран; учётку заводит администратор, вход по
логину.

![Экран входа](docs/screenshots/login.png)

**Дашборд** — 4 KPI со сравнением к прошлому периоду, блок средних времён
(до обнаружения / на диагностику / до эскалации), инциденты по дням, тепловая
карта «день недели × час обнаружения», разбивки по зонам / типам / сервисам,
блок «Эскалации и надёжность», пай топ-3 сервисов и скорость каналов связи.
Клик по любому графику ведёт к списку инцидентов, из которых он собран. Все числа
считает бэкенд (`/api/analytics/dashboard`), см. §4.3.

![Дашборд](docs/screenshots/dashboard.png)

**Список инцидентов** — поиск и фильтры (сервис, тип, SLA, зона, даты), бейдж
черновика, вердикт SLA в последней колонке. Фильтрация — на клиенте по уже
загруженному массиву.

![Список инцидентов](docs/screenshots/incidents.png)

**Карточка инцидента** — метрики (`data.metrics`, считает `IncidentMetrics`,
см. §4.1), полная хронология с временами шагов, тайминги, заглушка, причина,
влияние, зона ответственности, журнал изменений.

![Карточка инцидента](docs/screenshots/incident-detail.png)

**Создание инцидента** — общая информация, хронология из 4 обязательных шагов
(обнаружено · диагностика · передано ответственным · решена) с вставкой
промежуточных, журнал эскалаций, тип/критичность, заглушка, затронутые сервисы,
зоны, постмортем. Пока есть несохранённые правки — уход со страницы требует
подтверждения (§5).

![Создание инцидента](docs/screenshots/create-incident.png)

**Администрирование** (только роль `admin`) — порог SLA одним числом, управление
пользователями, четыре справочника (сервисы, зоны, типы, критичность) со
счётчиком использования; удаление заблокировано для используемого значения,
переименование каскадится в инциденты.

![Администрирование](docs/screenshots/admin.png)

---

*IMS · Системный анализ · по фактическому состоянию кода на 2026-09-08*
